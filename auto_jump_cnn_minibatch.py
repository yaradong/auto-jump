...
def start_train(sess,output,loss,train_op, x, y, keep_prob, learning_rate, saver,press_time_array, num, get_im_data_op, img_raw, batch_size_per_device):
    train_path = './train_data' + str(num) + '/'
    ...
    for i in range(max_epoc):
        for iter_id in range(iter_num_per_epoc):
            ...
            #print(str(i), str(iter_id), 'y_result:', y_result, 'press time:',  press_time_array[img_id], 'loss:',  loss_result)
            print(str(i), str(iter_id),  'loss:',  loss_result)
            sess.run(train_op, feed_dict={x:x_data,output:y_result, y:y_data, keep_prob: 0.6, learning_rate:0.0002})
            del y_result
            del loss_result
            del im_data
...
def jump(press_time):
    ...
    cmd = 'adb shell input swipe ' + str(left)+ ' '+ str(top) +' '\
          +str(right) +' '+ str(bottom) +' '+ str(int(press_time[0][0]))
    os.system(cmd)
    print(cmd)
...
def main(_):
    ...
    with tf.Session(graph=g) as sess:
        ...
        if int(train_or_test)==1:
            start_train(sess,output,loss,train_op, x, y, keep_prob, learning_rate, saver, press_time_array, num,get_im_data_op,img_raw, batch_size_per_device)
        elif int(train_or_test) == 2:
            # saver.restore(sess, "./save/mode.mod")
